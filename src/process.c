...
process_t* process_queue = NULL;   //queue for non-rt processes
process_t* current_process = NULL; //holds currently running process
realtime_t current_time = {0,0};   //holds the current time from start
process_t* ready_queue = NULL;     //holds queue for rt-processes
process_t* wait_queue = NULL;      //holds queue for rt-processes that are not ready
int process_deadline_miss = 0;     //counts deadlines missed
int process_deadline_met = 0;      //counts deadlines met
...
// append process (w/ state) to queue
void enqueue(process_t* *queue, process_t* process) {
	if (*queue == NULL) { //if queue is empty, sets it to process
		*queue = process;
		process->next = NULL;
	} else if (*queue == process_queue) { //appends to end of normal queue
		process_t* tmp = *queue;
		while(tmp->next != NULL) {
			tmp = tmp->next;
			process->next = NULL;
		...
		//appends to correct place of ready_queue by stepping through
		//and comparing deadline times
		process_t* tmp = *queue;
		...
		//appends to wait_queue by stepping through
		//and comparing arrival times
		process_t* tmp = *queue;
...
//checks wait_queue for processes that have become ready
//and moves them to ready_queue
void checkunready() {
	if (wait_queue != NULL) {
...
int process_rt_create(void (*f)(void), int n, realtime_t *start, realtime_t *deadline) {
	...
	realtime_t tmprt = {0,100};
	proc2->deadline = deadline; //&tmprt;
...
//enables pit and interrupts then begins processes
...
//handler for pit1, which keeps track of time in ms
void PIT1_IRQHandler(void) {
	...
	if(current_process->is_rt){
		if(getabstime(&current_time)>getabstime(current_process->deadline)){
...
//wait cycles if wait_queue not empty but process_queue and ready_queue empty
void waiter(){
	__enable_irq();
...
*/
	//disable interrupts so it is an atomic action
	__disable_irq();
	
	//checks for any waiting processes that have become ready
	checkunready();
		
	if(!cursp){
		//if there are no processes in any queue, exit scheduler
		if(!current_process && !process_queue && !ready_queue && !wait_queue){
			return NULL;
		} else if (current_process != NULL) {
			//if current process is_rt, check if met or missed deadline
			if(current_process->is_rt){
			...
			//if there are other things in queue, free current process
		...
		//otherwise re-queue current process
	...
	process_t *next_one;
	//selects from rt-process queue first then non-rt queue
	if (!ready_queue) {
	...
	if(!next_one){
		return NULL;
	} else { //updates current process, returns stack pointer
